...
**`classify` is self-healing, not fire-and-forget-only:** `actions/classification.ts`'s `classify()` used to unconditionally POST to start a new classification job on every call. This silently spawned a redundant second ELK run in practice — a completed-but-unsaved job does not block a new start the way a genuinely `RUNNING` one does, so calling `classify` again to "check on" a job that had already finished just started a different one instead. `classify` now reads the branch's current `latestClassificationJson` first (the same read `classificationStatus` exposes read-only, mirroring `pollClassificationStatusInVsCode()`'s cache-evict-then-fetch pattern) and reacts to what's actually there: attaches to and waits on an already-`RUNNING` job instead of starting a second one, accepts an already-`COMPLETED`-but-unsaved job immediately instead of re-running classification, and only starts fresh when nothing is pending (`SAVED`/`STALE`/`SAVE_FAILED`/never run). `classification-status` is the pure read-only version with no side effects at all, for a caller that just wants to check before deciding what to do next.

**`classify --wait` polls on the same tightening schedule as the webview**, not a flat interval: `buildClassificationPollSchedule()` (`classification.ts`, mirrored by the identically-named function in `apps/authoring-ui-vscode/app/components/edit/edit.js`) skips the first 90s (classification normally finishes in ~2 minutes, so nothing checked that early has a realistic chance of being done), then checks every 10s for 30s, then every 5s — a fixed schedule totaling exactly 3 minutes, which is also `classify`'s new default `--timeout` (down from a flat 600s). Once the schedule is exhausted, polling continues at its final 5s interval until the caller's own `timeoutSeconds` budget elapses, so an explicit `--timeout` larger than 180 (for a known-large branch where ELK genuinely takes 10-20+ minutes) still works. Kept deliberately in sync across the CLI and the webview — same underlying job, same expected timing, so both should feel consistent watching one complete.

**`validate-task` is self-healing too, same rationale as `classify()`.** Two validations running concurrently on the same task is pure waste — RVF work against the same branch state, twice — so `validate()` checks `task.latestValidationStatus` before starting anything: already `RUNNING`/`QUEUED`/`SCHEDULED` → attach to and wait on it instead of starting a second one, regardless of this call's own `wait` flag (finding an unresolved run in progress means it needs resolving, not a fire-and-forget response about a different, about-to-be-started run — identical reasoning to `classify()`'s running-job case). Otherwise, starts fresh as before.

**`validate-task --wait` gets its own, much longer schedule — task validation (RVF) routinely takes ~10 minutes or more.** `buildValidationPollSchedule()` (`classification.ts`) skips the first 5 minutes entirely, then checks once a minute for 5 minutes, then every 30s for 5 more — 15 minutes total, `validate-task`'s new default `--timeout` (also down from a flat 600s at a flat 5s interval). `classification-status`'s validation counterpart, `validation-status`, is the pure read-only check (no start, no wait) — reports `task.latestValidationStatus` directly, no cache-evict step needed (unlike classification, `scaService.js` has no equivalent cache to evict for validation status). The webview takes a different approach for the same reason validation is so much slower than classification: rather than an equivalent long-running auto-poll, `taskDetail.js` exposes a manual "Check Validation Status" button (`$scope.checkValidationStatus()`, VS Code mode only) that re-broadcasts the same `'reloadTask'` event `doValidate()` already uses, letting a human check on demand instead of the extension polling on their behalf for 10+ minutes. See `apps/authoring-ui-vscode/CLAUDE.md`'s "Classification/validation status polling in VS Code mode" section.

**The extension host holds a real STOMP/SockJS connection to authoring-services independent of any panel, and both the webview and the CLI's polling now wake from it.** `scaRelayManager.ts`'s `refreshScaRelay()` starts this connection (`notifications/scaNotificationRelay.ts` + `sockjsStompCodec.ts`, a hand-rolled SockJS-websocket-framing + STOMP-text-frame codec over the `ws` package) at extension activation if a session cookie already exists, and again after `pasteCookie`/`importChromeCookies` succeed — deliberately *not* tied to the Authoring Workbench panel's lifecycle, since the CLI's headless design can't depend on a human having that panel open (see "Deliberate design decision" above). Every parsed notification is published on an in-process bus (`notifications/notificationBus.ts`'s `emitNotification`/`onNotification`), which two independent things subscribe to: `extension.ts` forwards each one into the open webview via `postMessage({command: 'SCA_NOTIFICATION', ...})` (relayed there into `scaService.js`'s shared `handleNotificationPayload()` — see `apps/authoring-ui-vscode/CLAUDE.md`), and `classification.ts`'s `pollWithSchedule()` races each scheduled tick above against `waitForNotification()`'s matching promise, so `classify --wait`/`validate-task --wait` typically resolve on the real push rather than waiting out the next tick — the HTTP status check still always runs afterward as the authoritative source, so a missed or malformed notification just falls back to today's polling cadence rather than hanging.

**`watch-notification` is the same bus's third consumer, for events neither `classify` nor `validate-task` are waiting on.** `actions/watchNotification.ts` races `waitForNotification({entityType, projectKey, taskKey, branchPath})` — `entityType` optional, defaulting to any of `Classification`/`Validation`/`Rebase`/`Promotion`/`BranchState`/`BranchHead`/`Feedback`/`AuthorChange` (`scaService.js`'s handled entity types; `ConflictReport` stays an unhandled TODO there too) — against a plain timeout (default 60s), and returns whichever won: the matched notification payload, or `{timedOut: true}`. Like `classify`/`validate-task`, it resolves task context via `resolveTaskContext()`, so — also like them — it needs a resolvable project+task (explicit or the one currently open); there's no project-wide or branch-wide watch mode without a task.
